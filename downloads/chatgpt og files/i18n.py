...
# Languages the user can pick for transcript input (YouTube captions) and for asking questions.
# Code = BCP-47-ish short code used by youtube-transcript-api.
INPUT_LANGUAGES = [
    ("English", "en"),
    ("Hindi", "hi"),
    ("Spanish", "es"),
...
# Languages the LLM can answer in. Same set keeps the dropdown simple.
OUTPUT_LANGUAGES = [name for name, _ in INPUT_LANGUAGES]

# Per-language instructions injected into the system prompt so the LLM answers in the right language.
# Kept short and explicit — these are appended to the prompt template.
OUT_LANG_INSTRUCT = {
...
# Piper TTS voice selection. Keys are the language names above.
# These are the canonical piper voice IDs (downloadable from
# https://huggingface.co/rhasspy/piper-voices). Only the languages with
# reasonably small / fast voices are enabled by default.
PIPER_VOICES = {
...
# faster-whisper (via ctranslate2) expects plain ISO 639-1 codes, which is a
# different code space than the BCP-47-ish codes in INPUT_LANGUAGES above
# (e.g. "zh-Hans" is valid for youtube-transcript-api but not for Whisper,
# which just wants "zh"). Keep the mapping separate rather than reusing
# INPUT_LANGUAGES's codes directly.
WHISPER_LANG_CODES = {
    "English": "en",
    "Hindi": "hi",
    "Spanish": "es",
