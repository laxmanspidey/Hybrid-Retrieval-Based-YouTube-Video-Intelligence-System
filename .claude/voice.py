...
from i18n import piper_voice_for
...
def _load_whisper():
    ...
    log.info("Loading Whisper %s on %s (%s)", WHISPER_MODEL_SIZE, device, compute_type)
...
class VoiceIO:
    ...
    def transcribe(self, audio: np.ndarray, sample_rate: int = 16000) -> str:
        ...
        segments, _info = model.transcribe(
            audio,
            language="en",  # auto-detect would be "None" but we want consistent behavior
            beam_size=5,
            vad_filter=True,
        )
